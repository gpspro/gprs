...
REPORT_RID_DRIVER_KEY           = 1     # Driver Key (64-bit ID)
REPORT_RID_ADDITIONAL_IO        = 2     # Additional IO (Voltages, Inputs, Outputs, Analogs, etc)

class ReportDriverKey < BinData::Record
  endian :little

  uint64  :driver_key
end

class ReportAdditionalIO < BinData::Record
  endian :little
...
class ReportData < BinData::Record
  ...
  choice :data, :selection => :id do
    report_driver_key     REPORT_RID_DRIVER_KEY
    report_additional_io  REPORT_RID_ADDITIONAL_IO

    skip  :default, :length => 0, :value => "Unknown ID"
  end
end