$LOAD_PATH.unshift File.expand_path('../../lib', __FILE__)
require 'gprs'

require 'minitest/autorun'

# Unit ID: 1016 (test unit)
def report_single
  [0x28,0xC3,0x48,0xA1,0x11,0x5D,0x78,0x1F,0xF8,0x3,0xE9,0xF5,0x32,0xD7,0xBD,0x70,0x0,0x10,0x8D,0xCF,0x81,0x3,0xC8,0x6B,0x2,0x6D,0x87,0x1,0x0,0x0,0x0,0x1,0xC5,0x29]
end

# Unit ID: 4 (test unit)
def report_multiple
  [
    [0x28,0xC6,0x10,0xA8,0xA0,0xC1,0x93,0x22,0x1F,0x4,0xAD,0x95,0x3F,0x5C,0x25,0x9E,0x0,0x3,0x54,0x66,0x6,0x1D,0x78,0x90,0x2,0x6F,0x9F,0x1,0x24,0x4,0x0,0x0,0x0,0x1,0xD5,0x29],
    [0x28,0xC7,0x10,0xA8,0xA0,0xD1,0xA1,0x22,0x1F,0x4,0xB3,0x95,0x3F,0x5B,0x25,0x9E,0x0,0x3,0x60,0x90,0x7B,0x1F,0x78,0x90,0x2,0x6F,0x9F,0x1,0x24,0x4,0x0,0x0,0x0,0x1,0x52,0x29],
    [0x28,0x81,0x10,0xA8,0x0,0x0,0x0,0x0,0x0,0x4,0x0,0x0,0x80,0x0,0x0,0x0,0x0,0x10,0x88,0x5D,0x2,0x6F,0x9F,0x1,0x24,0x4,0x0,0x0,0x0,0x1,0xAA,0x29],
    [0x28,0x82,0x10,0xA8,0x20,0x59,0xB0,0x22,0x1F,0x4,0xB5,0x95,0x3F,0x5D,0x25,0x9E,0x0,0x10,0x8D,0x56,0x2,0x6F,0x9F,0x1,0x24,0x4,0x0,0x0,0x0,0x1,0x2A,0x29],
    [0x28,0x81,0x10,0xA8,0x0,0x0,0x0,0x0,0x0,0x4,0x0,0x0,0x80,0x0,0x0,0x0,0x0,0x10,0x88,0x50,0x2,0x6F,0x9F,0x1,0x24,0x4,0x0,0x0,0x0,0x1,0xA7,0x29]
  ]
end

# Packet #1: Report Type = 8, Ext Type = 240 (FC Transaction)
# Packet #2: Report Type = 6 (Compressed Report)
# Packet #3: Report Type = 9 (Compressed + Coded Report)
def report_other
  [
    "0x28 0xCA 0x10 0xA8 0xE0 0x1C 0xC0 0x4F 0x1D 0x64 0x85 0x97 0x3F 0x5F 0x27 0x9E 0x0 0x88 0x6E 0x90 0x7B 0x12 0x78 0x90 0xF0 0x3C 0x10 0x90 0x0 0x0 0x5C 0x2D 0x0 0x0 0x79 0x29",
    "0x28 0xCD 0x26 0xE0 0x2C 0x99 0xFC 0x1D 0x66 0xB1 0x6C 0x3F 0xA4 0x2D 0x9E 0x6 0x87 0x5D 0x1 0x12 0x10 0x90 0x82 0x90 0x55 0x5 0x0 0x0 0x6 0x55 0x3 0x82 0x0 0x6 0x55 0x5 0x82 0x82 0x6 0xD3 0x29",
    "0x28 0xE9 0x10 0xA9 0xE0 0x24 0x2C 0x5F 0x1D 0x64 0x8F 0x98 0x3F 0x61 0x27 0x9E 0x21 0x87 0x5 0x60 0x66 0x6 0x12 0x78 0x90 0x55 0x5 0xB 0x9 0x21 0x55 0x3 0x83 0x5 0x1A 0x59 0x25 0x17 0x81 0x93 0x0 0x67 0x29"
  ]
end

# Packet #1: Other packet (0)
# Packet #2: Report (1)
# Packet #3: Command (2)
# Packet #4: ACK (3)
# Packet #5: Program (4)
def packet_types
  [
    "0x41 0x54 0x5E 0x53 0x49 0x53 0x57 0x3D 0x30 0x2C 0x30 0x2C 0x22 0x31 0x30 0x2E 0x32 0x34 0x30 0x2E 0x31 0x2E 0x31 0x35 0x31 0x3A 0x31",
    "0x28 0x81 0x68 0x40 0x0 0x0 0x0 0x0 0xF 0x27 0x0 0x0 0x80 0x0 0x0 0x0 0x0 0x73 0x10 0x88 0x58 0x2 0x3 0xA1 0x1 0x7E 0x4 0xF9 0x29",
    "0x28 0xF4 0x0 0xF4 0x29",
    "0x28 0x17 0x17 0x29",
    "0x28 0x15 0x6 0x0 0x7 0x21 0x35 0x29"
  ]
end

# Includes a sample of ALL command type combinations
def commands
  [
    # Config Msg / Position Request
    "0x28 0x24 0x0 0x24 0x29",
    # Config Msg / Sendval Request (74)
    "0x28 0x14 0x14 0x4A 0x4A 0x29",
    # Config Msg / Param Request (1)
    "0x28 0x34 0x1F 0x1 0x2A 0x29",
    # Config Msg / FC Pump Request
    "0x28 0x4 0x50 0x9C 0x46 0x0 0x0 0x2D 0x0 0x0 0x0 0x7D 0x3 0x0 0x0 0xC4 0x10 0x8A 0x4 0x0 0x0 0x17 0x29",
    # Config Msg / Analog Extended Request
    "0x28 0x54 0x57 0x1 0x1 0x0 0x3 0x29",
    # Config Msg / Output Get Request
    "0x28 0x44 0x5A 0x1 0x1F 0x29",
    # Config Msg / Output Set Request
    "0x28 0x14 0x5B 0x1 0x2 0x1 0x1 0x0 0x0 0x4C 0x29",
    # Config Msg / Sleep Timeout Set Request
    "0x28 0x74 0x64 0x64 0x0 0x74 0x29",
    # Sendval Reply / Diag
    "0x28 0xD6 0x49 0x0 0x7 0x10 0xA8 0x1 0x9 0x1 0xA1 0x1 0x33 0x5 0x2F 0x29",
    # Sendval Reply / Diag V2
    "0x28 0x56 0x4A 0x37 0x0 0x7 0x10 0xA8 0x1 0x10 0x8D 0x10 0xA9 0xA4 0x1 0x30 0x7 0x7B 0x13 0x7 0x3 0x0 0x0 0x0 0x0 0xDF 0x29",
    # Param Reply / Modem APN Reply
    "0x28 0x10 0xA8 0x48 0xF 0x77 0x79 0x6C 0x65 0x73 0x73 0x2E 0x63 0x6F 0x6D 0x2E 0x61 0x74 0x74 0x7A 0x0 0x0 0x12 0x29",
    # Param Reply / Gateway IP/Port
    "0x28 0x18 0x49 0x10 0x8A 0xF0 0x1 0x97 0xC3 0x6 0xC3 0x6 0x3D 0x29"
  ]
end

def parse_report_single(log = true)
  if log
    puts ""
    puts "# Parsing 1 report packet..."
  end

  reports = GprsC.packet_parse(report_single, log)

  if log
    puts reports
    puts ""
  end
end

def parse_report_multiple(log = true)
  if log
    puts ""
    puts "# Parsing #{report_multiple.length} report packets..."
  end

  report_multiple.each do |packet|
    reports = GprsC.packet_parse(packet, log)
    if log
      puts reports
      puts ""
    end
  end
end

def parse_report_other(log = true)
  if log
    puts ""
    puts "# Parsing several non-additional-io packets..."
  end

  report_other.each do |packet|
    reports = GprsC.packet_parse(packet)
    if log
      puts reports
      puts ""
    end
  end
end

def detect_packet_types(log = true)
  if log
    puts ""
    puts "# Detecting types of #{packet_types.count} different packets..."
  end

  packet_types.each do |packet|
    type = GprsC.packet_type(packet)

    if log
      puts "Packet: #{packet}"
      puts "Packet Type: #{type}"
    end
  end
end

def parse_commands(log = true)
  if log
    puts ""
    puts "# Parsing all command packets (C Extension)..."
  end

  commands.each do |packet|
    command = GprsC.packet_parse(packet)
    if log
      puts command
      puts ""
    end
  end
end

def parse_command_c(log = true)
  if log
    puts ""
    puts "# Parsing single command packet (C Extension)..."
  end

  command = GprsC.packet_parse(commands[0])
  if log
    puts command
    puts ""
  end
end

def parse_command_kaitai(log = true)
  if log
    puts ""
    puts "# Parsing single command packet (Kaitai Struct)..."
  end

  command = GprsC.packet_parse_kaitai(commands[0])
  if log
    puts command.inspect
    puts ""
  end
end

def parse_commands_kaitai(log = true)
  if log
    puts ""
    puts "# Parsing all command packet types (Kaitai Struct)..."
  end

  commands.each do |packet|
    command = GprsC.packet_parse_kaitai(packet)
    if log
      puts command.inspect
      puts ""
    end
  end
end
